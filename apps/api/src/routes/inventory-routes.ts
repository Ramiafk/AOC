...
const returnBody = z.object({ lines: z.array(z.object({ partId: uuid, quantity: z.number().positive() })).min(1), reason: z.string().trim().min(3) });
const idParams = z.object({ id: uuid });
const siteParams = z.object({ siteId: uuid });
const alertQuery = z.object({ organizationId: uuid });
...
export function registerInventoryRoutes(app: FastifyInstance, contexts: RequestContextResolver, authorizer: RouteAuthorizer, inventory: ManageInventory): void {
  ...
  app.post("/v1/purchase-orders/:id/close-remainder", async request => { const context = await contexts.resolve(request); const params = idParams.parse(request.params); await authorizer.require(context, "parts.manage", await inventory.scopeForPurchaseOrder(context, params.id as EntityId)); return inventory.cancelRemainder(context, params.id as EntityId); });
  app.post("/v1/purchase-orders/:id/returns", async request => { const context = await contexts.resolve(request); const params = idParams.parse(request.params); const body = returnBody.parse(request.body); await authorizer.require(context, "parts.manage", await inventory.scopeForPurchaseOrder(context, params.id as EntityId)); return inventory.returnToSupplier(context, params.id as EntityId, body.lines as Array<{ partId: EntityId; quantity: number }>, body.reason); });
  app.get("/v1/sites/:siteId/replenishment-alerts", async request => { const context = await contexts.resolve(request); const params = siteParams.parse(request.params); const query = alertQuery.parse(request.query); await authorizer.require(context, "parts.manage", { organizationId: query.organizationId as EntityId, siteId: params.siteId as EntityId }); return inventory.replenishmentAlerts(context, query.organizationId as EntityId, params.siteId as EntityId); });
}
