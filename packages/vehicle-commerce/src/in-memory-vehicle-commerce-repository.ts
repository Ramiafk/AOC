...
export class InMemoryVehicleCommerceRepository implements VehicleCommerceRepository {
  ...
  async sell(value:VehicleSaleProps,stockItem:VehicleStockItemProps){const snapshot={items:[...this.items],publications:[...this.publications],sales:[...this.sales],flashSales:[...this.flashSales]};try{await this.saveStockItem(stockItem);this.publications=this.publications.map(item=>item.stockItemId===stockItem.id&&item.status==="published"?{...item,status:"withdrawn"}:item);this.flashSales=this.flashSales.map(item=>item.stockItemId===stockItem.id&&item.status==="scheduled"?{...item,status:"closed",closedReason:"sold",closedBy:value.soldBy,closedAt:value.soldAt}:item);this.sales.push(value);}catch(error){this.items=snapshot.items;this.publications=snapshot.publications;this.sales=snapshot.sales;this.flashSales=snapshot.flashSales;throw error;}}
